...
$(document).ready(function() {
    ...
    $(document).on('click', '.single_item', function() {
        ...
        draw_table_for_order += (row_number > 0) ? '' : '<div class="single_order fix" style="font-size:20px;" id="order_for_item_' + item_id + '">';
    ...
    $(document).on('click', '#dine_in_button', function() {
        var total_items_in_cart = $('.order_holder .single_order').length;
        if(total_items_in_cart == ""){
            $("#delivery_charge").val(2.5);
            $("#delivery_order").val(0);
            $("#tax_charge").val(10);
        } else{
            $("#delivery_charge").val(2.5);
    ...
    $('#place_order_operation').on('click', function() {
        ...
        var tax_charge2 = ($('#tax_charge').val() != "") ? parseFloat($('#tax_charge').val()).toFixed(2) : parseFloat(0).toFixed(2);
	var tax_charge = parseFloat(tax_charge2 * sub_total / 100).toFixed(2);	
	    ...
	    console.log(sale_id);
            ...
            //place_order_ajax(order_object, sale_id);
    ...
    $('#direct_invoice').on('click', function() {
        ...
        var tax_charge2 = ($('#tax_charge').val() != "") ? parseFloat($('#tax_charge').val()).toFixed(2) : parseFloat(0).toFixed(2);
	var tax_charge = tax_charge2 * sub_total / 100;
...
//edit
function place_order_ajax(order_object, sale_id) {

    $.ajax({
        url: base_url + "Kitchen/place_order_ajax",
        method: "POST",
        data: {
            order: order_object,
            sale_id: sale_id,
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var key in response) {
                ...
                $order_list_left += '<span id="open_orders_order_status_' + response[key].sales_id + '" style="display:none;">' + response[key].order_status + '</span><p style="display:none;font-size: 16px;text-align:left;width: 125px;float: left;">Order: <span class="running_order_order_number">' + order_name + '</span></p><img src="' + base_url + 'assets/images/right-arrow.png" style="float: right;width: 13px;margin: 2px;transition: .25s ease-out;" class="running_order_right_arrow" id="running_order_right_arrow_' + response[key].sales_id + '">';
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var key in response) {
                ...
                $order_list_left += '<span id="open_orders_order_status_' + response[key].sales_id + '" style="display:none;">' + response[key].order_status + '</span><p style="display: none;font-size: 16px;font-weight: bold;text-align:left;width:125px;float:left">Order: <span class="running_order_order_number">' + order_name + '</span></p><img src="' + base_url + 'assets/images/right-arrow.png" style="float: right;width: 13px;margin: 2px;transition: .25s ease-out;" class="running_order_right_arrow" id="running_order_right_arrow_' + response[key].sales_id + '">';
                $order_list_left += '<p>Table: <span class="running_order_table_name">' + tables_booked + '</span></p>';
                $order_list_left += '<p>Waiter: <span class="running_order_waiter_name">' + waiter_name + '</span></p>';
                $order_list_left += '<p>Customer: <span class="running_order_customer_name">' + customer_name + '</span></p>';
                $order_list_left += '</div>';
...
function update_kot_ajax(sale_id){
    $.ajax({
        url: base_url + "Sale/update_kot_ajax",
        method: "POST",
        data: {
            sale_id: sale_id,
            csrf_test_name: $.cookie('csrf_cookie_name')
...
function update_bill_ajax(sale_id){
    $.ajax({
        url: base_url + "Sale/update_bill_ajax",
        method: "POST",
        data: {
            sale_id: sale_id,
            csrf_test_name: $.cookie('csrf_cookie_name')
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var key in response.items) {
                ...
                draw_table_for_order += '<div class="single_order_column forth_column fix"><input type="" name="" placeholder="Amt or %" class="special_textbox" id="percentage_table_' + this_item.food_menu_id + '" value="' + this_item.menu_discount_value + '" disabled><i style="cursor:pointer;" class="fa fa-tags edit_diskon" id="edit_diskon_' + this_item.food_menu_id + '"></i></div>';
...
function get_details_of_a_particular_order_for_modal(sale_id) {
    $.ajax({
		...
		console.log(response.items);
            ...
            $("#sub_total_discount_order_details").html(response.total_item_discount_amount);
            ...
            $("#delivery_charge_order_details").html(response.delivery_order);
	    $("#service_charge_order_details").html(response.delivery_charge);
	    $("#tax_charge_order_details").html(response.tax_charge);
...
function arrange_info_on_the_cart_to_modify(response) {
    ...
    if (response.order_type == '1') {
        ...
        $("#delivery_charge").val(2.5); //service charge
        $("#delivery_order").val(0); //delivery charge
        $("#tax_charge").val(10); //tax charge
        ...
        $("#delivery_charge").val(0); //service charge
        $("#delivery_order").val(0); //delivery charge
        $("#tax_charge").val(10); //tax charge
