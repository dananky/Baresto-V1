...
class Sale extends CI_Controller {
    ...
    public function printSale($sale_id)
     {
	     ...
	     $payment_id = $this->input->post('payment_method_type');
	     $payment = '';
	     if($payment_id == 4){
		$payment = 'Card';
		...
		$payment = 'KREDIVO';
	     ...
	     $printer -> text('ORDER TYPE: '.$order_name."\n");
             ...
             $sub_total = $data->sub_total + $data->total_item_discount_amount;
             $total_discount_amount = $data->total_item_discount_amount;
             $service_charge = $data->delivery_charge;
             $delivery_charge = $data->delivery_order;
             $tax_charge = $data->tax_charge;
             $printer -> setEmphasis(true);
             if($order_type == 'A'){
                $printer -> text(new item('Sub Total', number_format($sub_total,0,",",".")));
                $printer -> feed();
                $printer -> setEmphasis(false);
                $printer -> text(new item('Disc Amt', '-'.number_format($data->total_item_discount_amount,0,",",".")));
                $printer -> text(new item('Service Charge(2,5%)', number_format($data->delivery_charge,0,",",".")));
                $printer -> text(new item('Tax Charge(10%)', number_format($data->tax_charge,0,",",".")));
                $printer -> setEmphasis(true);
                $printer -> feed();
                ...
                $printer -> setEmphasis(false);
                $printer -> text(new item('Disc Amt', '-'.number_format($data->total_item_discount_amount,0,",",".")));
                //$printer -> text(new item('Delivery Charge', number_format($data->delivery_order,0,",",".")));
                $printer -> text(new item('Tax Charge(10%)', number_format($data->tax_charge,0,",",".")));
                $printer -> setEmphasis(true);
                $printer -> feed();
             }
            ...
            //  $printer -> setEmphasis(false);
            //  $printer -> text(new item('Disc Amt (%)', '-'.number_format($data->total_item_discount_amount,0,",",".")));
	        //  $printer -> text(new item('Service Charge', number_format($data->delivery_charge,0,",",".")));
            //  $printer -> text(new item('Delivery Charge', number_format($data->delivery_order,0,",",".")));
	        //  $printer -> text(new item('Tax Charge', number_format($data->tax_charge,0,",",".")));
            //  $printer -> setEmphasis(true);
            //  $printer -> feed();
             
             // $printer -> selectPrintMode(Printer::MODE_DOUBLE_WIDTH);
             $printer -> text(new item('Grand Total', number_format($data->total_payable,0,",",".")));
             $printer -> text(new item($payment, number_format($given_amount,0,",",".")));
	...
	//$data['cek_data'] = $this->get_all_information_of_a_sale(390);
	...
	$data['delivery_order'] = trim($order_details->delivery_order);
	$data['tax_charge'] = trim($order_details->tax_charge	);
    ...
    public function update_kot_ajax(){
    	$sale_id = $this->input->post('sale_id');
	$this->printKOT($sale_id);
    }    
    
    public function update_bill_ajax(){
    	$sale_id = $this->input->post('sale_id');
	$this->printBill($sale_id);
    }  
    ...
    public function printKOT($sale_id)
    {
        try {
