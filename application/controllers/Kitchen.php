...
class Kitchen extends CI_Controller {
    ...
    public function printKitchen($sale_id) {
        $connector = new NetworkPrintConnector("192.168.0.232", 9100);
    ...
    public function printKitchen2($sale_id) {
        $connector = new NetworkPrintConnector("192.168.0.231", 9100);
    ...
    public function getKitchenPrint() {
        ...
        // var_dump($print);die;
	//$data = array();        
	//$data['sales_details'] = $this->get_all_information_of_a_sale($previous_id);
	//$data = array();        
	//$data['data'] = $this->get_order_details_kitchen_ajax($previous_id);
	...
	$waiter_id = $order_details->waiter_id;
	$data = array();
	foreach($order_details->items as $item){
		$menu_name = $item->item__name;
		array_push($data,$menu_name);
	}

        $this->printKitchen3($customer_id,$waiter_id,$sale_id,$data);
